...
Task<void> ProtocolBuffer::write_command(int fd, uint8_t seq) {
  ...
  send_buf.add_u24l(this->get_length());
  send_buf.add_u8(seq);
...
uint64_t ProtocolBuffer::remove_u48l() {
  uint64_t v = 0;
  this->remove(&v, 6);
  return v;
}
...
uint64_t ProtocolBuffer::remove_varint() {
  ...
  if (v < 0xFB) {
    ...
    return this->remove_u16l();
  } else if (v == 0xFD) {
    return this->remove_u24l();
  } else if (v == 0xFE) {
    return this->remove_u64l();
  } else if (v == 0xFF) {
...
void ProtocolBuffer::add_u24l(uint32_t v) {
  this->add(&v, 3);
}

void ProtocolBuffer::add_u48l(uint64_t v) {
  this->add(&v, 6);
}
...
uint64_t ProtocolStringReader::get_varint() {
  ...
  if (v < 0xFB) {
    ...
    return this->get_u16l();
  } else if (v == 0xFD) {
    return this->get_u24l();
  } else if (v == 0xFE) {
    return this->get_u64l();
  } else if (v == 0xFF) {
    throw runtime_error("length-encoded int is invalid (0xFF)");
  }
  return v;
